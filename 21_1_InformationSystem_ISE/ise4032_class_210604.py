...
def function_test(arg1, arg2):
    print('this is a function')

def function_sum(x: int, y: int): #  괄호안에 이렇게 넣으면 변수 2개밖에 못받음, so *args를 넣어야 원하는 만큼의 변수 개수를 얻을 수 있음
    """
    이 함수는 두 정수를 더하는 함수입니다.
    :param x: 첫번째 정수 (무조건 정수만 넣어주세요)
    :param y: 두번째 정수 (무조건 정수만 넣어주세요)
    :return: 두 정수의 합
    """
    # 위처럼 적어두면 함수 정의가 뜸! 굳굳
    return x+y

def function_sum1(*args):
    # *args에 입력된 변수들은 튜플 형식으로 입력됨
    total = 0
    for thisNum in args:
        total += thisNum
    return total
...
# *args는 그냥 모든 변수를 튜플로 받음 -> 변수에 대한 순서? 정렬? 없이 걍 순차로 때려박음
def function_sum2(**kwargs):    # **kwargs는 딕셔너리 형태로 받음
    print('hello')